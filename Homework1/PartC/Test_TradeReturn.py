...
class Test_TradeReturn(unittest.TestCase):
    '''
    classdocs
    '''
    ...
    def test(self):
        ...
        if not os.path.isdir(testPath):
            os.mkdir(testPath)
        testFile = os.path.join(testPath, "test_trades.gz")
        with gzip.open(testFile, 'wb') as f:
            fmt = '>QIIf'
            content = struct.pack(fmt, 1000, 1, 1, 1.0)
            f.write(content)
            content = struct.pack(fmt, 1000, 1, 1, 1.0)
            f.write(content)
            content = struct.pack(fmt, 2000, 1, 1, 1.5)
            f.write(content)
            content = struct.pack(fmt, 3000, 1, 1, 3.0)
            f.write(content)
            content = struct.pack(fmt, 3900, 1, 1, 6.0)
            f.write(content)
            content = struct.pack(fmt, 4100, 1, 1, 9.0)
            f.write(content)
        
        tr = TradeReturn(testPath)
        ls = tr.GetReturn("test", 1000)
